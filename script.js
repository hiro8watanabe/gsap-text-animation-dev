document.addEventListener("DOMContentLoaded", () => {
  ...
  let str3 = text3.innerHTML.trim().split("");
  ...
  text3.innerHTML = str3.reduce((acc, curr) => {
    //渡ってきた半角スペースを&nbsp;へ置換し、再びcへ代入(/\s+/は正規表現の半角スペース)
    curr = curr.replace(/\s+/, "&nbsp;");
    //cにstrの文字が一文字ずつ入ってくる
    return `${acc}<span class="char3">${curr}</span>`; //テンプレートリテラルで繋いだものをループさせて次々concatStrへ格納
  }, "");

  const btn = document.querySelector(".button");
  let char = document.querySelector(".char").className;
  let char2 = document.querySelector(".char2").className;
  let char3 = document.querySelector(".char3").className;
